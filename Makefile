...
compiler:
	...
	-@sudo /sbin/ldconfig; sudo /sbin/ldconfig -p | grep -q "libmysqlclient.so " || { which zypper && sudo zypper -n--gpg-auto-import-keys in libmysqlclient-devel || { which apt-get && { sudo apt-get --assume-yes install libmysqlclient-dev; } || { which dnf && sudo yum install -y mysql-devel; } }; sudo /sbin/ldconfig; }
