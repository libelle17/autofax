...
GROFFCHECK={ rpm -q groff >/dev/null 2>&1 || dpkg -s groff groff-base >/dev/null 2>&1;} || sudo zypper -n --gpg-auto-import-keys in groff || { which apt-get && { sudo apt-get --assume-yes install groff groff-base; } || { sudo which dnf && { sudo dnf -y install groff groff-base; } || { sudo which yum && sudo yum -y install groff groff-base; } } }
...
compiler:
	...
	-@which $(CCName) >/dev/null 2>&1 || { which zypper && { sudo zypper lr | grep 'g++\|devel_gcc' || sudo zypper ar http://download.opensuse.org/repositories/devel:/gcc/`cat /etc/*-release | grep ^NAME= | cut -d'"' -f2 | sed 's/ /_/'`_`cat /etc/*-release | grep ^VERSION_ID= | cut -d'"' -f2`/devel:gcc.repo; sudo zypper -n --gpg-auto-import-keys si $(CCInst); } || { which apt-get && sudo apt-get --assume-yes install build-essential || { sudo which dnf && { sudo dnf -y install make automake gcc-c++ kernel-devel; } || { which yum && yum install -y make automake gcc-c++ kernel-devel; } } } }
	-@sudo /sbin/ldconfig; sudo /sbin/ldconfig -p | grep -q "libmysqlclient.so " || { which zypper && sudo zypper -n --gpg-auto-import-keys in libmysqlclient-devel || { which apt-get && { sudo apt-get --assume-yes install libmysqlclient-dev; } || { sudo which dnf && { sudo dnf -y install mysql-devel; } || { which yum && sudo yum install -y mysql-devel; } } }; sudo /sbin/ldconfig; }
	-@test -f /usr/include/tiff.h || { which zypper && sudo zypper -n --gpg-auto-import-keys in libtiff-devel || { which apt-get && { sudo apt-get --assume-yes install libtiff-dev; } || { sudo which dnf && { sudo dnf -y install libtiff-devel; } || { which yum && sudo yum install -y libtiff-devel; } } } }
