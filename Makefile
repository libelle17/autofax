...
GROFFCHECK={ rpm -q groff >/dev/null 2>&1 || dpkg -s groff groff-base >/dev/null 2>&1;} || { which zypper >/dev/null 2>&1 && { sudo zypper -n --gpg-auto-import-keys in groff 2>/dev/null;true;} || { which apt-get && { sudo apt-get -y --force-yes install groff groff-base;true;} || { sudo which dnf && { sudo dnf -y install groff groff-base;true;} || { sudo which yum && sudo yum -y install groff groff-base;true;} } } }
...
compiler:
	...
	-@which $(CCName) >/dev/null 2>&1 || { which zypper >/dev/null 2>&1 && { sudo zypper lr | grep 'g++\|devel_gcc' || sudo zypper ar http://download.opensuse.org/repositories/devel:/gcc/`cat /etc/*-release | grep ^NAME= | cut -d'"' -f2 | sed 's/ /_/'`_`cat /etc/*-release | grep ^VERSION_ID= | cut -d'"' -f2`/devel:gcc.repo; sudo zypper -n --gpg-auto-import-keys in -y gcc gcc-c++ $(CCInst);true;} || { which apt-get && { sudo apt-get -y --force-yes install build-essential linux-headers-`uname -r`;true;} || { sudo which dnf && { sudo dnf -y install make automake gcc-c++ kernel-devel;true;} || { which yum && yum install -y make automake gcc-c++ kernel-devel;true;} } } }
	-@if { sudo /sbin/ldconfig; ! sudo /sbin/ldconfig -p | grep -q "libmysqlclient.so ";} || ! test -f /usr/include/mysql/mysql.h; then { which zypper >/dev/null 2>&1 && { sudo zypper -n --gpg-auto-import-keys in libmysqlclient-devel;true;} || { which apt-get && { sudo apt-get -y --force-yes --reinstall install libmysqlclient-dev;true;} || { sudo which dnf && { sudo dnf -y install mysql-devel;true;} || { which yum && sudo yum install -y mysql-devel;true;} } }; sudo /sbin/ldconfig;}; fi
	-@test -f /usr/include/tiff.h || { which zypper >/dev/null 2>&1 && { sudo zypper -n --gpg-auto-import-keys in libtiff-devel;true;} || { which apt-get && { sudo apt-get -y --force-yes install libtiff-dev;true;} || { sudo which dnf && { sudo dnf -y install libtiff-devel;true;} || { which yum && sudo yum install -y libtiff-devel;true;} } } }
