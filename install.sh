...
TU="{ rpm -q make >/dev/null 2>&1 ||dpkg -s make >/dev/null 2>&1;} ||
    ...
    cd '$PWD' && make && make install;"
#erg=1; which rpm &&{ rpm -q make >/dev/null 2>&1;erg=$?;true;} || which dpkg &&{ dpkg -s make >/dev/null 2>&1;erg=$?;true;}; (exit $erg)|| 
echo $TU
{ which sudo >/dev/null && id -Gn $USER|grep -qw 'sudo\|wheel\|admin'; } &&
  { sh -c "$TU";true;} ||
{ echo "Bitte geben Sie bei der Frage das Passwort von 'root' ein:";su -c "apt-get -y install sudo;usermod -aG sudo $USER;"; echo "Bitte loggen Sie sich jetzt aus und nochmal ein, wechseln Sie nach '$PWD' und rufen Sie das Script dann nochmal auf";}
