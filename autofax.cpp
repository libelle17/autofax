	...
	if (!obocrgeprueft) {
		...
		for(int aru=0;aru<2;aru++) {
			if (obprogda("tesseract",obverb,oblog,&tpfad)) {
				...
				switch (linstp->ipr) {
					case zypper:
						systemrueck("zypper ar -f --gpgcheck-allow-unsigned-repo https://download.opensuse.org/repositories/home:Alexander_Pozdnyakov/openSUSE_Leap_$(lsb-release -r|cut -f2)/home:Alexander_Pozdnyakov.repo",obverb,oblog);break; // 29.8.20
					default: break;
				}
