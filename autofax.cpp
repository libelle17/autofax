...
void paramcl::lieskonfein()
{
  ...
  // <<"lieskonfein nach obcapi: "<<(int)obcapi<<endl;
    ...
    if (obcapi) {if (gconf[lfd].gelesen) gconf[lfd].hole(&maxcapiv); else rzf=1;} lfd++;
    if (obcapi && obhyla) {if (gconf[lfd].gelesen) gconf[lfd].hole(&maxhylav); else rzf=1;} lfd++;
    ...
    if (cgconfp[++lfd].wert.empty() || rzf) {
      ...
      while (1) {
        ...
        for(size_t akt=0;akt<nzmzn;akt++) {
          string zwi=holstring(string("Zielmuster Nr. ")+ltoan(akt+1)+Tx[T_beim_letzten_nichts_eingeben],&nzmconfp[2*akt].wert);
...
int paramcl::pruefhyla()
{
  ...
  for(unsigned versuch=0;versuch<2;versuch++) {
    ...
    if (hylafehlt) {
      ...
      /*
      if (hyinstart==hysrc) {
        if (shylafaxd->machfit(obverb-1,oblog)) hylafehlt=0;
      } else {
      */
...
//      }
    ...
    if (1) {
      for (uchar iru=0;iru<3;iru++) {
        fglaeuftnicht=!this->sfaxgetty->obslaeuft(obverb,oblog);
  ...
  *ausgp<<"Capi: "<<schwarz;
  ...
  if (capistat!=fehlend) {
    ...
    if (p1) {
      ...
      size_t cs=sizeof cconf/sizeof*cconf;
      confdat cpconf(suchtxt,cconf,cs,obverb);
      if (1) {
...
#define direkt
  ...
  systemrueck(string("tac \"")+datei+"\" 2>/dev/null | grep -m 1 \""+this->hmodem+sep+jobid+sep+"\" | cut -f 14",1,oblog,&grueck); 
  if (grueck.size()) {
    gefunden=1;
    *erg=grueck[0];
#else
    systemrueck(string("tac \"")+datei+"\" 2>/dev/null | grep -m 1 \""+this->hmodem+sep+jobid+sep+"\"",obverb,oblog,&grueck); // ggf. broken pipe error
    if (grueck.size()) KLA
      vector<string> tok;
    aufSplit(&tok,&grueck[grueck.size()-1],sep);
#endif // direkt else
#else // mitgrep else
    mdatei f(datei.c_str(),ios::in|ios::binary); 
    string zeile;
#define rueckwaerts
#ifdef rueckwaerts
    char ch;
