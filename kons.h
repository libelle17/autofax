...
//    uchar obgefunden=0; // braucht man nicht, ist in argcl
    ...
    const uchar obno{0}; // ob auch die Option mit vorangestelltem 'no' eingefuegt werden soll
		...
		const uchar sonderrf{0}; // 1 = in rueckfragen individuelle Behandlung
		const uchar virteinzutragen(/*schAcl<optcl>**/void *schlp,int obverb);
...
//		void virtloeschomaps(/*schAcl<optcl>**/void *schlp);
		...
		optcl(const string& pname,const void* pptr,const par_t art, const int kurzi, const int langi, TxB* TxBp, const long Txi,
				const uchar wi, const long Txi2, const string rottxt, const int iwert, const uchar woher, const string& Txtrf=string(), const uchar obno=0,const string* refstr=0,const uchar* obfragz=0,fnhcliztyp fnobfragz=0, fnhclztyp fnnachhz=0,fnhclztyp fnvorhz=0,const uchar sonderrf=0);
...
//    schAcl<WPcl> agcnfA; // Gesamtkonfiguration
	...
	public:
		virtual void fu1(),fu2(),fu3(),fu4(),fu5(),fu6(),fu7(),fu8(),fu9(),fu10();
